...
function Stop-Ids([int[]]$ids, [int]$timeoutSec = 5) {
  ...
  foreach ($id in $ids) {
    try {
      ...
      if ($p) {
        Write-Host ("Deteniendo Mosquitto por PID ({0})..." -f $id)
...
function Get-PidsByCommandLine($cfg) {
  $ids = @()
  try {
    $procs = Get-CimInstance Win32_Process -Filter "Name='mosquitto.exe'"
    foreach ($pr in $procs) {
...
if (Test-Path $pidPath) {
  $MosqPid = (Get-Content $pidPath | Select-Object -First 1).Trim()
  if ($MosqPid -match '^\d+$') {
    $stopped = Stop-Ids @([int]$MosqPid)
    if ($stopped) { Remove-Item $pidPath -Force -ErrorAction SilentlyContinue }
  }
}

# 2) CommandLine
if (-not $stopped -and (Test-Path $confPath)) {
  $ids = Get-PidsByCommandLine $confPath
...
if ($stopped) {
  Write-Host "Mosquitto detenido."
} else {
  Write-Warning "No encontré proceso mosquitto para detener. Si se inició elevado, corre este script en una consola elevada."
}
